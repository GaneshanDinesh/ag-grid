...
describe('PdfCreator', () => {
    it('merges a base title string into a style-only override', () => {
        const creator = new PdfCreator() as any;
        const overrideTitle: PdfCell = {
            data: { value: null },
            style: { fontSize: 12 },
        };

        const merged = creator.mergeDocumentTitle('Quarterly Results', overrideTitle) as PdfCell;
    ...
    it('resolves document title colours to computed values', () => {
        ...
        const resolved = creator.resolveDocumentTitle(title) as PdfCell;
