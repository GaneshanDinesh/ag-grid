...
test('parses various param value types and formats', () => {
    ...
    expect(parseThemeCode(`{ fontSize: myVar, backgroundColor: "#fff" }`)).toMatchObject({
        ...
        variableWarnings: [
            'Invalid value for fontSize: myVar (expected a literal value e.g. string or object, not javascript code)',
        ],
    });
    ...
    expect(parseThemeCode(`{ fontSize: myFunction(), backgroundColor: "#fff" }`)).toMatchObject({
        ...
        params: { backgroundColor: '#fff' },
        variableWarnings: [
            'Invalid value for fontSize: myFunction() (expected a literal value e.g. string or object, not javascript code)',
        ],
    });
    ...
    expect(
        ...
        params: { backgroundColor: '#fff' },
        variableWarnings: [
            'Invalid value for fontSize: calculate(a, b, [`template ${foo()} `, d]) (expected a literal value e.g. string or object, not javascript code)',
        ],
    });
    ...
    expect(
        ...
        params: { backgroundColor: '#fff' },
        variableWarnings: [
            'Invalid value for fontSize: this is an extremely long string over several line... (expected a literal value e.g. string or object, not javascript code)',
        ],
    });
});
...
test('detects parts anywhere in code with last-wins for same feature', () => {
    ...
    // Code without .withParams() still detects parts
    expect(parseThemeCode(`const theme = themeQuartz.withPart(iconSetAlpine);`).parts).toEqual([iconSetAlpine]);
