...
export function parseThemeCode(code: string): ParseThemeResult {
        ...
        if (validParams.has(token.value) && tokens[i + 1]?.value === ':') {
            ...
            if (result.valid) {
                ...
                variableWarnings.push(
                    `Invalid value for ${paramName}: ${result.invalidCode} (expected a literal value e.g. string or object, not javascript code)`
                );
            }
        } else {
            i++;
    ...
    if (Object.keys(params).length === 0) {
        return {
            success: false,
            error: 'Could not find any theme parameters. Expected code like: themeQuartz.withParams({ backgroundColor: "#fff" })',
            params: undefined,
            parts: undefined,
            variableWarnings: undefined,
        };
    ...
    return { success: true, params, parts: extractParts(), variableWarnings };
